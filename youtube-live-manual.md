...
### 2. OBSで配信開始
1. Macで OBS を起動（プロファイル: YouTube Live）
2. 「配信開始」ボタンを押す
3. OBSログで `Connection to rtmp://...successful` を確認
...
- **確認項目**:
  1. Macのデフォルト出力デバイス → BlackHole 2ch か確認
     ...
     ```
  2. OBSに「BlackHole Audio」入力ソースがあるか確認
  3. Macの音量がミュートでないか確認
     ```
...
1. OBSで「配信停止」
2. screenセッション停止: `screen -X -S shibu-chat quit`
3. YouTube Studioで配信を終了
...
| /tmp/shibu-chat.log | Mac /tmp/ | 実行ログ |
| /tmp/shibu-voice-queue/ | Mac /tmp/ | 生成された音声ファイル |
...
| ElevenLabs v3 | TTS音声生成 | 環境変数 `ELEVENLABS_API_KEY` |

## OBS WebSocket (補助)

ポート4455で利用可能。BlackHole Audio入力ソースの追加などに使える。
