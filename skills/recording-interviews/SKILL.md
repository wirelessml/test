...
2. **🚨 アプリ側のスピーカーも「複数出力装置」にする（最重要。2026-06-15 エボルカ面談で相手の声を全ロスした原因）**。Meet/Zoom は**システム既定を無視して独自のスピーカー設定**を使うため、それが EarPods 等に直結していると相手の声が BlackHole を経由せず、**相手トラックが丸ごと無音(-91dB)**になる。Meet: 設定⚙ > 音声 > スピーカー = 複数出力装置 ／ Zoom: 設定 > オーディオ > スピーカー = 複数出力装置
3. **🚨 本番直前に「アプリの実音」でテストする**。`afplay`/`say` のシステム音テストが通っても、アプリ独自出力までは検証できない（2026-06-15 はシステム音テストだけ通して本番で失敗）。Meet/Zoom/YouTube を実際に鳴らし、相手トラックに音が乗る（−91dB でない）ことを確認してから本番へ
4. ⚠️ **イヤホン使用時は保険が効かない**：相手の声が BlackHole を経由しないと、イヤホンは耳内再生でマイクにも乗らず**相手の声はどこにも残らない**（スピーカー出しなら最悪マイクが拾うが、イヤホンだと完全ロス）
5. Zoom はアプリ不要、**Chrome ブラウザ参加で可**（zoom cask は sudo 必須でリモート導入不可、6/11 実証）
6. **M1 の蓋を閉じない**（蓋閉じで内蔵マイクが HW 切断される、6/11 教訓）

## 罠と対策（ハマったら最初にここを見る)

| 症状 | 原因と対策 |
|---|---|
| **相手トラックが丸ごと無音 (-91dB)＝面接官の声が録れない** | **アプリ(Meet/Zoom)のスピーカーがシステム既定(複数出力装置)を無視し EarPods 等へ直結** → 相手の声が BlackHole に届かない。イヤホンだと耳内再生でマイクにも乗らず**完全ロス**（2026-06-15 エボルカ面談で実証＝面接官の声を全ロス、自分の声と画面のみ残存）。対策: **アプリ内スピーカーも複数出力装置に設定**＋**本番前にアプリ実音でテスト**（システム音 `afplay`/`say` テストだけでは検知できない） |
