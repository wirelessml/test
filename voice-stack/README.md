...
> ⚠️ **V3 アップデート (4/28 10:15)**: 勝間本人が同日朝に **Scribe 単体 vs Whisper+GPT120B 後処理** を実機検証 → **Scribe 単体圧勝** と結論 (ID 2048933979420790843)。理由: (1) 前後文脈の精度差、(2) LLM が直しきれない元 Whisper の限界、(3) **Scribe ハルシネーションゼロ**。
>
> このリポジトリの `gpt120b_vs_llama70b.py` は本来用途を喪失したが、**Llama 70B vs GPT 120B**（勝間が回した組み合わせと逆ペア）の検証用としては未消化。再現性の記録としても保持。
