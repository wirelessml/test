...
- [ ] **Joy-Con 2 ペアリング実機検証**（Switch 2 から切断 → SYNC 長押し → dotnet run）
- [ ] GATT characteristic を subscribe してデータダンプ確認
- [ ] enable_std/ext 送信 → 60 byte パケット受信を確認
- [ ] InputPacket.cs のパース結果（ボタン・スティック・光学・ジャイロ）が maruta コードと一致するか照合
