...
4/22 の夕方、コワーキングスペースに置いてあるしゅん先生 PC（Windows 11 メイン作業機）の Plextor M9P+ 512GB が突然死した。NVMe コントローラごと逝ったので、再起動しても BIOS から見えない完全沈黙。2 時間前に作っていたクローン HDD が命綱になり、Seagate ST2000LM015（SMR の 2TB HDD）から起動して延命運用に入った。
...
旧 Seagate ST2000LM015（SMR HDD）との比較:
...
| 日付 | イベント |
|---|---|
| 4/22 18:20 | Plextor M9P+ 512GB 突然死、NVMe コントローラ障害 |
...
| 4/29 16:45 | CrystalDiskMark 3374 MB/s、Plextor 時代に完全復帰 |

実質コスト: **¥10,267**（はばタンPay+ 50% プレミアム適用後の Acer FA100 価格）

これで 2026 年 NAND 高騰の SSD 危機を、しゅん先生 PC 単位で完全突破した。Plextor 時代の速度に戻ったというよりは、新品なので **実質的にむしろ上**だ。Plextor は 5 年使って劣化していたはずなので。
