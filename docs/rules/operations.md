...
- **ユーザーへの画面表示は日本語のみ**（2026-06-13 ユーザー指示）。コマンド・コード・ファイルパス・固有名詞など不可避な英単語は可。地の文の説明文や状況報告は日本語で書く。
- **Gmailの読み取り・下書き作成はCodex側で行う**（2026-06-13 ユーザー指示）。`gws` の認証済みGmail機能を使い、メール本文確認・スレッド確認・下書き作成まで実行する。送信は明示指示がある場合のみ行う。
- **🔖 符牒「シェアハウスでルームシェア」＝Gmailの同名ラベルを確認する合図**（2026-06-13 ユーザー指示で明文化）。ユーザーがこのフレーズを言ったら、Gmail ラベル「シェアハウスでルームシェア」（重要・要対応メール＝就活の面接連絡/応募やり取り等をまとめた個人用ラベル）の最新スレッドを確認し、要対応・要注目を報告する。`label:シェアハウスでルームシェア` で取得（ラベルID指定は空振りするのでラベル名で検索）。DAZN等と空目しないこと
- **記事の削除や一括更新をする前は、必ず件数を教えて確認を取ること。それ以外は確認不要**
- **🚨「時間かかりすぎ」「まだ？」等の発言は中止シグナルとして扱う**（6/10 iMovieテストの教訓）。同じ重い経路（GUI自動化・承認待ち等）を再試行せず、即座に止まって「現状＋残り見込み＋中止/別ルートの選択肢」を一言で報告する。続行はユーザーが選んだ場合のみ
- **computer-use のアクセス承認待ちは無言でブロックしない**。リクエスト発行時に「画面に承認ダイアログが出ています」と一言告げる。承認が来ない場合に再リクエストを繰り返さない
