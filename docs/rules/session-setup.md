...
- **セッション開始時にターミナルを常に最前面に設定する**（ユーザーに案内して実行）
  ...
  - 元に戻す場合はレベルを0に設定
- **モデル設定（2026-06-14 更新）**: **標準 = Opus 4.8**（`~/.claude/settings.json` の `"model": "opus"`、max effort）。Fable 5 は予定の 6/22 を待たず使用不可となり移行済み（6/14 ユーザー確認）。旧「Fable 期間中はモデル切替禁止（claude-fable-5 固定）」ルールは失効。モデル変更が要るときはユーザー確認の上で行い、`/model opusplan`（思考=Opus・実行=Sonnet 自動切替）も選択可
