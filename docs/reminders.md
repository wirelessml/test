...
- **2026/07/13（DAZN 視聴終了・対応不要）**: 6/13 加入（DAZN スタンダード・1,980 円）→ **即日「解約」済み**（一時停止ではない）。DAZN は「解約申請から30日後に終了」ルールのため、契約直後の解約で **視聴は ~7/13 まで・追加課金なし**（1,980 円の1回のみ）。W杯は準々決勝（~7/12）までカバー、準決勝（日本時間 7/15・16）以降は DAZN 対象外。決勝 7/20・3位決定戦 7/19 は NHK 総合で無料生中継（準決勝は地上波未確定、現状 BSP4K 録画のみ）。※「8/1 再課金」は一時停止を選んだ場合の話で、本件（解約済み）には**無関係**。何もしなくてよい

- ~~**2026/06/22（月）**: Claude Fable 5 無料期間終了~~ → **Fable 5 は 6/14 までに使用不可**（予定より前倒しで終了、6/14 ユーザー確認）。標準モデルは **Opus 4.8** に移行済み（`~/.claude/settings.json` の `"model": "opus"`、max effort）。Fable 期間に始めた資産化は通常作業として継続（@docs/projects/fable5-assets-plan.md）
- **2026 秋（9〜11 月目安）**: **povo 2.0 の eSIM（080-3108-7536）を長女・結花の新 iPhone に移行予定**（6/12 仲氏言明）。手順: povo2.0 アプリで機種変更（eSIM 再発行）→ 新 iPhone で開通。※povo MNP 自体は実施済み（6/12 本人確認、Y!mobile は自動解約済みで ¥4,158 リスクは消滅）
- ~~ふゆ×くら！お金部（DMM サロン）退会~~ **6/12 ユーザー指示でリマインダー削除（以後本人管理）**。サロン分析の詳細は @docs/journal/2026-06-05.md に残存
...
- ✅ 実施済み（6/12 本人確認）**2026/05/29（木）〜5/30（金）**: **povo 2.0 へ MNP ワンストップ転出**（080-3108-7536 番号維持、Y!mobile 自動解約）
