...
- **2026/07/12（DAZN フル視聴の最終日）**: 6/13 加入（DAZN スタンダード・PayPay・1,980 円）。**フル視聴は 7/12 まで**（W杯は準々決勝まで見られる。準決勝＝日本時間 7/15・16 以降は対象外）。7/12 から一時停止を設定済み。決勝 7/20・3位決定戦 7/19 は NHK 総合で無料生中継（準決勝は地上波に来ず BS4K 録画のみ）
- **🚨 2026/08/01 より前に DAZN を「解約」する**: 今回は解約でなく **一時停止**＝放置すると **8/1 に再課金（約 1,980 円）**。W杯を見終わる 7/12 頃にその場で「解約」して再課金を止めるのが安全。一時停止のままにしないこと

- **2026/06/22（月）**: **Claude Fable 5 無料期間（従量課金なし）終了** — それまでに「6/23 以降も残る資産」（スキル・ハーネス・ドキュメント・重い執筆）への変換を完了させる。終了後はモデルを `/model opusplan` に戻す（6/12 戦略決定、@docs/journal/2026-06-12.md）
