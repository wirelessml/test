...
## 15. セッション分割でコンテキスト品質を維持する

### 問題：長時間セッションは品質が劣化する

Claude Code を長く使い続けると、コンテキストウィンドウにノイズが蓄積する。古いエラーログ、試行錯誤の履歴、不要になった中間結果……。これらが混ざった状態でAIが推論するため、**セッションが長いほど出力品質が下がる**。

### 解決策：30〜45分ごとにセッションを切る
...
| 観点 | 効果 |
|------|------|
| **品質** | ノイズのない新鮮なコンテキストで推論精度が回復する |
| **トークン** | 圧縮済みの要約から再開するので、累積トークン消費を大幅に削減 |
| **集中力** | 人間側も30〜45分で区切ることで、作業の方向性を再確認できる |

### 「文脈が失われるのでは？」→ CLAUDE.md があれば問題ない

セッションを切ると文脈が消えることを心配するかもしれない。しかし：

- **CLAUDE.md** にプロジェクトの規約・構成・方針が書いてある
- **git の差分** からこれまでの変更内容がわかる
- **要約** で直近の作業状況を引き継げる

この3つが揃っていれば、引き継ぎコストはほぼゼロ。むしろ蓄積したノイズがリセットされる分、**切った方が品質が上がる**。

### 実践Tips

```bash
# セッション終了前に要約を依頼
> 今の状況をまとめて。次のセッションに引き継ぎたい。
...
- 要約には「完了したこと」「未完了のこと」「次にやるべきこと」を含めてもらう
- 大きなタスクは最初から30分単位に分割して計画すると、自然なセッション分割になる

---

## 16. CLAUDE.md は短く書け

### 原則：AIが既に知っていることは書かない

CLAUDE.md に書くべきは **AIが知らない、このプロジェクト固有の情報だけ**。一般的なベストプラクティスやプログラミングの常識は不要。AIはそんなこと既に知っている。
...
# 制約
- APIレスポンスは必ず Zod スキーマでバリデーション
- DB マイグレーションは Drizzle を使う（Prisma 禁止）
- コンポーネントは app/components/ 配下にフラットに配置
...
# ❌ プロジェクトの歴史
- 2024年1月にReactからNext.jsに移行しました
- 以前はREST APIでしたがGraphQLに変更しました
```

### ルール数の上限：150〜200個

ある弁護士（Claude Code ヘビーユーザー）の検証によると、**ルールが150〜200個を超えると Claude Code の従順度が落ちる**。ルールが多すぎるとAIが優先順位をつけられなくなり、重要なルールが埋もれる。
...
### 短い CLAUDE.md の方が効く理由

| 長い CLAUDE.md | 短い CLAUDE.md |
|----------------|----------------|
| ルールが多すぎて優先順位が不明確 | 少数のルールに集中できる |
| 一般論がノイズになる | プロジェクト固有の情報だけ |
| 毎回大量のトークンを消費 | コンテキストを圧迫しない |
| 矛盾するルールが生まれやすい | 矛盾が起きにくい |

**迷ったら削る。** 「これを書かなかったらAIは間違えるか？」と自問する。答えがNoなら不要。

### Skills によるプログレッシブ・ディスクロージャー

CLAUDE.md を短くするだけでは足りない。情報量が多いプロジェクトでは、**Skills に分離して必要なときだけロードする** のが正解。
...
セッション開始
  ↓
CLAUDE.md（全ルール・全コンテキスト）を一括読み込み
  ↓
25,000 トークン消費 😱
  ↓
実際に使うのはその一部だけ
...
【改善：プログレッシブ・ディスクロージャー】

セッション開始
  ↓
CLAUDE.md（最小限のルール + Skills の一覧）を読み込み
  ↓
955 トークン消費 ✓
  ↓
必要な Skill だけオンデマンドでロード
```

**26倍のトークン効率改善**（25,000 → 955トークン）。桁が違う。
...
.claude/
├── skills/
│   ├── deploy.md          ← デプロイ手順（デプロイ時のみロード）
│   ├── db-migration.md    ← DB マイグレーション手順
│   ├── api-conventions.md ← API 設計規約
│   ├── test-patterns.md   ← テストの書き方
...
必要なときに /deploy, /db-migration 等で呼び出す。
```

### なぜ効くのか

| 静的ロード | プログレッシブ・ディスクロージャー |
|-----------|----------------------------------|
| 全情報を毎回読み込む | 必要なときだけロード |
| 25,000 トークン | 955 トークン |
| 不要な情報がノイズになる | コンテキストがクリーン |
| ルール上限に達しやすい | 実質無制限に情報を持てる |

CLAUDE.md は「目次」、Skills は「各章」。全部まとめて1ファイルに書くのではなく、構造化して必要なときだけ参照する。

### プロンプトキャッシュを壊す3つのアンチパターン

Anthropic API にはプロンプトキャッシュがある。同じプロンプトのプレフィックスが一致すれば、キャッシュされたトークンは **90%オフ** で再利用される。逆に言えば、プレフィックスが変わるとキャッシュが無効化され、全トークンがフルプライスで再計算される。

**絶対にやってはいけないこと：**

```
❌ 1. System Prompt にタイムスタンプを入れる

  "You are a helpful assistant. Current time: 2026-03-28T14:32:05Z"
                                               ^^^^^^^^^^^^^^^^
                                               毎回変わる → キャッシュ毎回ミス

❌ 2. セッション途中でツールを追加・削除する

  ツール定義は System Prompt の一部。
  途中で変わるとプレフィックスが崩れてキャッシュ無効化。

❌ 3. セッション途中でモデルを切り替える

  Opus → Haiku に切り替えると、キャッシュは全再構築。
  「Haiku の方が安い」つもりが、キャッシュ再構築コストで
  Opus を続行するより高くつくことすらある。
```

```
【キャッシュが効いている状態】

リクエスト1: [System Prompt][Tool定義][会話履歴]     → 全額
リクエスト2: [System Prompt][Tool定義][会話履歴][新]  → 差分だけ課金（90%オフ）
リクエスト3: [System Prompt][Tool定義][会話履歴][新]  → 差分だけ課金（90%オフ）

【キャッシュが壊れた状態（タイムスタンプ変更等）】

リクエスト1: [System Prompt v1][Tool定義][会話履歴]   → 全額
リクエスト2: [System Prompt v2][Tool定義][会話履歴]   → 全額 💸
リクエスト3: [System Prompt v3][Tool定義][会話履歴]   → 全額 💸💸
```

特にモデル切り替えは直感に反する罠。「重い処理が終わったから安いモデルに切り替えよう」とすると、それまでの会話履歴のキャッシュが全て無駄になる。**同じモデルで走り切る方がトータルで安い**ケースが多い。

### コンテキスト管理 ＝ コスト管理 ＝ 品質管理

ここまでのTipsは全て **コンテキストをいかに小さく・クリーンに保つか** という一点に集約される。
...
| やること | 防ぐもの |
|---------|---------|
| セッションを短く切る | コンテキストの肥大化 |
| CLAUDE.md を短く保つ | 静的なコンテキスト汚染 |
| Skills をオンデマンドロード | 不要な情報の読み込み |
| Prompt Caching を壊さない | 再計算コスト |
| モデルを使い分ける | 過剰なコスト |

これらを理解している人と理解していない人で、**同じ月額$20でもできることの幅が全く違う**。

コンテキストが小さければトークン消費が減り、コストが下がる。同時に、AIが参照する情報がクリーンになり、出力品質が上がる。逆にコンテキストが肥大化すると、コストが増え、品質が落ちる。この三位一体を意識するだけで、Claude Code の実効性能は劇的に変わる。
...
- 2026-03-28: セッション分割でコンテキスト品質維持セクションを追加
- 2026-03-28: CLAUDE.md は短く書けセクションを追加
- 2026-03-28: Skills によるプログレッシブ・ディスクロージャーを追記
- 2026-03-28: プロンプトキャッシュを壊すアンチパターンを追記
- 2026-03-28: コンテキスト管理＝コスト管理＝品質管理の三位一体まとめを追記
